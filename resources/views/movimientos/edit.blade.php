...
<div class="container">
    ...
    <form action="{{ route('movimientos.update', $movimiento) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <select name="producto_id" id="producto_id" class="form-control" required>
                @foreach($productos as $producto)
                    <option value="{{ $producto->id }}" {{ $movimiento->producto_id == $producto->id ? 'selected' : '' }}>{{ $producto->nombre }} (Stock: {{ $producto->stock }})</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="number" name="cantidad" id="cantidad" class="form-control" value="{{ $movimiento->cantidad }}" min="1" required>
        </div>

        <div class="mb-3">
            <label for="precio_unitario" class="form-label">Precio unitario (opcional)</label>
            <input type="number" step="0.01" name="precio_unitario" id="precio_unitario" class="form-control" value="{{ $movimiento->precio_unitario }}">
        </div>

        <div class="mb-3">
            <label for="motivo" class="form-label">Motivo</label>
            <input type="text" name="motivo" id="motivo" class="form-control" value="{{ $movimiento->motivo }}">
        </div>

        <div class="mb-3">
            <label for="referencia" class="form-label">Referencia</label>
            <input type="text" name="referencia" id="referencia" class="form-control" value="{{ $movimiento->referencia }}">
        </div>
